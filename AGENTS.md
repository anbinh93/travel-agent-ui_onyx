...
- Pluggable agent registry in `backend/onyx/agents/registry.py`
- Implement new agents by creating a module that exposes a LangGraph runner and registering an `AgentDefinition`
- Agent metadata is exposed via `/agenthub/agents`, enabling UI selection without manual wiring
- Environment variables (for example, Tavily or Gemini keys) are automatically populated from the project `.env`
